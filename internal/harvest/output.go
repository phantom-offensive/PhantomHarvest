...
// OutputNetscape writes decrypted cookies in Netscape cookie file format,
// ready for use with `curl -b cookies.txt`, Burp Suite, or browser extensions.
...
func OutputNetscape(findings []Finding, path string) error {
	...
	f.WriteString("# Generated by PhantomHarvest\n\n")

	count := 0
	for _, finding := range findings {
		if finding.Type != "cookie" {
			continue
		}
		// Key format: "Browser | host | name | expires_utc"
		parts := strings.SplitN(finding.Key, " | ", 4)
		if len(parts) < 3 {
			continue
		}
		host := parts[1]
		...
		// Determine expiry — use stored value or 0 (session)
		expires := "0"
		if len(parts) == 4 && parts[3] != "" && parts[3] != "0" {
			// Chrome stores expiry as microseconds since 1601-01-01.
			// Convert to Unix timestamp (seconds since 1970-01-01).
			// Offset = 11644473600 seconds between the two epochs.
			if chromeMicros, err2 := parseInt64(parts[3]); err2 == nil && chromeMicros > 0 {
				unixSec := chromeMicros/1_000_000 - 11_644_473_600
				if unixSec > 0 {
					expires = fmt.Sprintf("%d", unixSec)
				}
			}
		}
		...
		secure := "FALSE"
		if strings.HasPrefix(strings.ToLower(name), "__secure") || strings.HasPrefix(strings.ToLower(name), "__host") {
			secure = "TRUE"
		}

		fmt.Fprintf(f, "%s\t%s\t/\t%s\t%s\t%s\t%s\n",
			host, subdomains, secure, expires, name, value)
		count++
	}

	if count == 0 {
		fmt.Fprintln(os.Stderr, "  [!] No decrypted cookies found for Netscape export (run with -decrypt-browsers)")
		return nil
	}
	fmt.Printf("  \033[32m[+]\033[0m Exported Netscape cookies: %s (%d cookies)\n", absPath(path), count)
	return nil
}
