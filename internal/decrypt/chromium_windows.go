...
// DPAPI-only format. Returns an error (rather than panicking) on empty
// or malformed blobs — Login Data has NULL password_value rows for every
// site the user told Chrome "never save" on.
func chromiumDecryptValue(blob, key []byte) ([]byte, error) {
	...
	if len(blob) > 3 && (bytes.Equal(blob[:3], []byte("v10")) || bytes.Equal(blob[:3], []byte("v11"))) {
		...
		if err != nil {
			return nil, fmt.Errorf("%w: AES-GCM open (Chrome v20+ app-bound encryption?)", err)
		}
		return pt, nil
	}
